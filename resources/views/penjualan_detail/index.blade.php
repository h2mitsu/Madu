...
@section('breadcrumb')
    @parent
    <li class="active">Sale transaction</li>
@endsection
                ...
                <div class="row">
                    ...
                    <div class="col-lg-4">
                        <form action="{{ route('transaksi.simpan') }}" class="form-penjualan" method="post">
                            ...
                            <div class="form-group row">
                                <label for="bayar" class="col-lg-2 control-label">Total Due</label>
                                <div class="col-lg-8">
                                    <input type="text" id="bayarrp" class="form-control" readonly>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="diterima" class="col-lg-2 control-label">Money Received</label>
                                <div class="col-lg-8">
                                    <input type="number" id="diterima" class="form-control" name="diterima" value="{{ $penjualan->diterima ?? 0 }}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="kembali" class="col-lg-2 control-label">Change</label>
                                <div class="col-lg-8">
...
<script>
    ...
    $(function () {
        ...
        $(document).on('input', '.quantity', function () {
            ...
            if (jumlah < 1) {
                $(this).val(1);
                alert('Amount cant be less than one');
                return;
            }
            if (jumlah > 10000) {
                $(this).val(10000);
                alert('Amount cant be more than 10000');
                return;
            }
    ...
    function loadForm(diskon = 0, diterima = 0) {
        ...
        $.get(`{{ url('/transaksi/loadform') }}/${diskon}/${$('.total').text()}/${diterima}`)
            .done(response => {
                ...
                $('.tampil-bayar').text('Total Due: Rp. '+ response.bayarrp);
                $('.tampil-terbilang').text(response.terbilang);

                $('#kembali').val('Rp.'+ response.kembalirp);
                if ($('#diterima').val() != 0) {
                    $('.tampil-bayar').text('Change: Rp. '+ response.kembalirp);
